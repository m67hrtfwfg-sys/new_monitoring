...
async def send_telegram(message):
    ...
    if not bot_token or not chat_id:
        print("⚠️ خطا: توکن یا چت‌آیدی پیدا نشد!")
        return
...
async def check_and_save_site(session, s, db):
    ...
    for attempt in range(max_retries):
        ...
        try:
            async with session.get(s.url, timeout=10) as r:
                ...
                if r.status == 200:
                    final_status = "SLOW" if r_time > latency_threshold else "UP"
                    final_r_time = r_time
                    break 
        except Exception:
    ...
    old_status = last_status.get(s.url)
    if old_status != final_status:
        ...
        elif final_status == "UP" and old_status in ["DOWN", "SLOW"]:
...
async def monitor_loop():
    while True:
        ...
        try:
            ...
            recent_downs = db.query(SiteStatus).filter(SiteStatus.status == "DOWN").order_by(SiteStatus.id.desc()).limit(5).all()
            ...
            for ws in list(connections):
                try:
                    await ws.send_json({"history": history, "uptime": uptime_data, "logs": logs, "system": sys_stats})
                except:
                    connections.add(ws) 
...
async def get_data(user: str = Depends(get_current_user)):
    ...
    recent_downs = db.query(SiteStatus).filter(SiteStatus.status == "DOWN").order_by(SiteStatus.id.desc()).limit(5).all()
    for log in recent_downs:
        logs.append({"site": log.site_url, "time": log.timestamp.strftime("%Y-%m-%d %H:%M:%S"), "status": "DOWN"})
